  ...
  try {
    ...
    if (!response.data.values || response.data.values.length === 0) {
      ...
      const headers = isAdmin
        ? ['First Name', 'Middle Name', 'Surname', 'Mobile', 'Email', 'Gender', 'Spreadsheet ID', 'Password', 'Security Code']
        : ['First Name', 'Middle Name', 'Surname', 'Mobile', 'Email', 'Gender', 'Password', 'Security Code'];
...
app.post('/api/register/admin', async (req, res) => {
  try {
    ...
    pendingRegistrations.set(email, { firstName, middleName, surname, mobile, email, gender, spreadsheetId, hashedPassword, securityCode, isAdmin: true });
...
app.post('/api/register/user', async (req, res) => {
  try {
    ...
    const userSpreadsheetId = adminData.data.values[adminData.data.values.length - 1][6];

    await ensureHeaders(userSpreadsheetId, false); // Ensure headers in user sheet

    // Store user details temporarily
    pendingRegistrations.set(email, { firstName, middleName, surname, mobile, email, gender, userSpreadsheetId, hashedPassword, securityCode, isAdmin: false });
    ...
    if (purpose === 'registration') {
      ...
      if (isAdmin) {
        ...
        for (let i = 1; i < adminRows.length; i++) {
          if (adminRows[i][8] === code) { // Check if the security code matches any admin's code
            assignedSpreadsheetId = adminRows[i][6]; // Get the admin's spreadsheet ID
            assignedSecurityCode = adminRows[i][8]; // Get the correct security code
            break;
          }
        }
      ...
      if (isAdmin) {
        ...
        const adminRows = adminData.data.values;
        const userRow = adminRows.find(row => row[4] === email && row[8] === code);
        ...
        const userRow = userData.data.values.find(row => row[4] === email && row[7] === assignedSecurityCode);
        ...
        // Verify password
        const passwordMatch = await bcrypt.compare(req.body.password, userRow[6]);
        if (!passwordMatch) {
          return res.status(401).json({ message: 'Invalid password' });
        }
...
app.post('/api/forgot-password', async (req, res) => {
  try {
    ...
    const userRow = data.data.values.find(row => row[4] === email);
    if (!userRow) {
    ...
    const rowIndex = data.data.values.findIndex(row => row[4] === email);
    await sheets.spreadsheets.values.update({
      spreadsheetId,
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (isAdmin) {
      ...
      if (userRow[8] !== securityCode) {
        return res.status(401).json({ message: 'Invalid security code' });
      }

      const passwordMatch = await bcrypt.compare(password, userRow[7]);
      if (!passwordMatch) {
        return res.status(401).json({ message: 'Invalid password' });
      }
      ...
      const adminRow = adminData.data.values.find(row => row[8] === req.body.securityCode);
      if (!adminRow) {
        return res.status(401).json({ message: 'Invalid security code' });
      }

      // Get the corresponding spreadsheet ID
      const userSpreadsheetId = adminRow[6];
      ...
      const userDetailRow = userData.data.values.find(row => row[4] === email && row[7] === password && row[8] === req.body.securityCode);
      if (!userDetailRow) {
...
async function getAdminSpreadsheetId() {
  ...
  return adminData.data.values[adminData.data.values.length - 1][6];
}
